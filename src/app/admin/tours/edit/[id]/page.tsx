      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            category: tourData.category || 'Krishna Pastimes',
          ...
          setError(result.message || 'Failed to load tour details.');
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(result.message || 'Failed to update spiritual tour');
      }
    } catch (error) {
      console.error('Error updating tour:', error);
      alert('Failed to update spiritual tour. Please try again.');
    } finally {
    ...
    return (
      <div className="min-h-screen p-8 flex flex-col items-center justify-center bg-gray-50">
        <div className="bg-red-50 text-red-600 p-6 rounded-2xl border border-red-100 max-w-md text-center shadow-lg">
          ...
          <Link href="/admin/tours" className="inline-block bg-orange-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow hover:bg-orange-600 transition">
            Back to Tours
          </Link>
        </div>
      </div>
      ...
      <header className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-800">Edit Spiritual Tour</h1>
              <p className="text-xs text-gray-400">Modify tour details and day itineraries</p>
            </div>
          </div>
          <button
